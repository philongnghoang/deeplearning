...
def build_classification_graph(model, checkpoint, num_classes,batch_size=None):
    ...
    with tf.Graph().as_default() as tf_graph:
        with tf.Session(config=tf_config) as tf_sess:
            if batch_size==None:
              ...
              tf_input = tf.placeholder(tf.float32, [batch_size, net.input_height, net.input_width, 3],
                    name=input_name)
